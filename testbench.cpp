...
int main()
{
    ...
    int port_no = -1, numberOf_submit = -1, temp_int = -1;
    string filePath = "", server_str = "";
    char temp_char = 0;
    
    //askTo_user(&server_str, &port_no, &filePath, &numberOf_submit);
    server_str = "127.0.0.1";
    port_no = 40001;
    filePath = "/home/anseungchan/workspace/s_p/simulator.out";
    numberOf_submit = 15;
    ...
    for(int i=0; i<numberOf_submit; i++)
    {
        ...
        sp_client.connect_toServer();
        sp_client.checkConnect();
        sp_client.requestJobInfo(temp_int);
