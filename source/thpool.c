...
static int thread_init (thpool_* thpool_p, struct thread** thread_p, int id){
	...
	*thread_p = (struct thread*)malloc(sizeof(struct thread));
    ...
    pthread_t *thread;
    void* arg;
    pthread_create(thread, NULL, &thread_do, arg);
    */
...
* @return nothing
*/
static void* thread_do(void* argument){
    struct thread* thread_p = (thread*) argument;
	...
	act.sa_flags = 0;
	act.sa_handler = (__sighandler_t)thread_hold; // TOCHECK
